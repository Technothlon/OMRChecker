...
if(PRELIM_CHECKS):
    ...
    OMRresponseDict,final_marked,multimarked,multiroll = readResponse("H",ALL_WHITE,name = "ALL_WHITE", save = None, noAlignment=True)
    ...
    OMRresponseDict,final_marked,multimarked,multiroll = readResponse("H",ALL_BLACK,name = "ALL_BLACK", save = None, noAlignment=True)
...
for filepath in allOMRs:
    ...
    OMRcrop = getROI(inOMR,filename_noext+ext, closeUp=args["closeUp"], noMarkers=args["noMarkers"])
    if(OMRcrop is None):
...
print('Finished Checking %d files in %d seconds(~%d minutes) =>  ~%2f sec/OMR:)' % (filesCounter,timeChecking,round(timeChecking/float(filesCounter+1),2),round(timeChecking/float(60*filesCounter+1),2)))
